...
export const AllCategoryTable = () => {
  const [category, setcategory] = useState([]);
  const {
    isLoading: fetchcategoryIsLoading,
    isError: fetchcategoryIsError,
  ...
  useEffect(() => {
    if (fetchcategoryData && fetchcategoryData.data) {
      setcategory(fetchcategoryData.data);
    }
  }, [category]);
  ...
  const columns = [
    ...
    {
      name: "Date Added",
      selector: (row) => row.category_created_at,
      sortable: true,
    },
    
    {
      name: "Status",
      selector: (row) => row.category_status,
      sortable: true,
    },
 
    {
      ...
      cell: (row) => (
        ...
        <Link to={'/admin-dashboard/addcategory/1'} className="view_product_btn_link">
  ...
  return (
    <>
      <DataTable
        columns={columns}
        data={
          fetchcategoryData && fetchcategoryData.data
